@if ($agenda->obPlacements->isNotEmpty())
    <div>
            ...
            @foreach ($agenda->obPlacements as $placement)
                @if ($placement->legislativeSession)
                    <li class="rounded-lg border border-slate-100 px-3 py-2 dark:border-slate-700">
                        <a href="{{ route('ob.document.maker', $placement->legislativeSession) }}" class="font-medium text-brand-700 hover:underline dark:text-brand-200">
