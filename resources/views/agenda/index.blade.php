...
<div id="agenda-search" class="splis-agenda-index" data-search-url="{{ route('agenda.search') }}">
    <div class="splis-page-header">
        ...
        @can('create', App\Models\AgendaItem::class)
            <a href="{{ route('agenda.create') }}" class="splis-btn-primary">
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <button type="button" class="splis-stat splis-stat--gold splis-stat--clickable text-left" data-agenda-stat-filter data-filter-status="pending">
            <p class="splis-stat-label">Pending</p>
            <p class="splis-stat-value" id="agenda-stat-pending">{{ number_format($stats['pending']) }}</p>
            <p class="splis-stat-meta">Awaiting action</p>
        </button>
        <button type="button" class="splis-stat splis-stat--brand splis-stat--clickable text-left" data-agenda-stat-filter data-filter-due-soon="1">
            <p class="splis-stat-label">Due soon</p>
            <p class="splis-stat-value" id="agenda-stat-due-soon">{{ number_format($stats['due_soon']) }}</p>
            <p class="splis-stat-meta">Within 7 days</p>
        </button>
        <button type="button" class="splis-stat splis-stat--green splis-stat--clickable text-left" data-agenda-stat-filter data-filter-status="done">
            <p class="splis-stat-label">Accomplished</p>
            <p class="splis-stat-value" id="agenda-stat-done">{{ number_format($stats['done']) }}</p>
            <p class="splis-stat-meta">Done items</p>
        </button>
        <button type="button" class="splis-stat splis-stat--sky splis-stat--clickable text-left" data-agenda-stat-filter data-filter-status="lapsed">
            <p class="splis-stat-label">Lapsed</p>
            <p class="splis-stat-value" id="agenda-stat-lapsed">{{ number_format($stats['lapsed']) }}</p>
            <p class="splis-stat-meta">Deemed approved</p>
        </button>
    </div>

    <div class="mb-4 flex flex-wrap gap-2">
        <button type="button" class="splis-agenda-chip" data-agenda-quick-filter data-filter-reset="1">All</button>
        <button type="button" class="splis-agenda-chip" data-agenda-quick-filter data-filter-status="pending">Pending</button>
    ...
    <form id="agenda-search-form" class="splis-filter-panel splis-filter-panel--accent">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div>
                <label class="splis-label" for="agenda-filter-status">Status</label>
                <select name="status" id="agenda-filter-status" class="splis-select">
        ...
        <details id="agenda-advanced-filters" class="splis-filter-advanced">
            ...
            <div class="splis-filter-advanced-panel">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <div class="splis-combobox" data-combobox data-options='@json($senders)'>
                        <label class="splis-label" for="agenda-filter-sender">Sender</label>
                        <div class="splis-combobox-control">
                            <input type="text" name="sender" id="agenda-filter-sender" class="splis-input splis-combobox-input" placeholder="Municipality or office" autocomplete="off" data-combobox-input>
                    ...
                    <div class="splis-combobox" data-combobox data-options='@json($committees)'>
                        <label class="splis-label" for="agenda-filter-committee">Committee</label>
                        <div class="splis-combobox-control">
                            <input type="text" name="committee" id="agenda-filter-committee" class="splis-input splis-combobox-input" placeholder="Committee referred" autocomplete="off" data-combobox-input>
                    ...
                    <div class="splis-combobox" data-combobox data-options='@json($outcomes)'>
                        <label class="splis-label" for="agenda-filter-outcome">Outcome</label>
                        <div class="splis-combobox-control">
                            <input type="text" name="outcome" id="agenda-filter-outcome" class="splis-input splis-combobox-input" placeholder="Outcome" autocomplete="off" data-combobox-input>
                            <button type="button" class="splis-combobox-trigger" data-combobox-trigger aria-label="Show outcome options">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5"/></svg>
                            </button>
                            <div class="splis-combobox-panel" data-combobox-panel>
                                <div class="splis-combobox-list" data-combobox-list role="listbox" aria-label="Outcome options"></div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <label class="splis-label">Series (Year)</label>
                        <input type="number" name="series" class="splis-input" placeholder="Year" min="1900" max="2100">
                    </div>
                    <div>
