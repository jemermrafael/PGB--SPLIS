                                    ...
                                    <div class="mt-4 grid grid-cols-1 gap-4">
                                        ...
                                        @if ($link['field'] === 'pdf_final_journal' && ($finalJournalCandidateAgendas ?? collect())->isNotEmpty())
                                            @php
                                                ...
                                                $journalTaggedIds = collect($finalJournalTaggedIds ?? [])->map(fn ($id) => (int) $id)->all();
                                                if (old('final_journal_agenda_ids') !== null) {
                                                    $checkedFinalJournalAgendaIds = collect(old('final_journal_agenda_ids'))->map(fn ($id) => (int) $id)->all();
                                                } elseif ($journalTaggedIds !== []) {
                                                    $checkedFinalJournalAgendaIds = $journalTaggedIds;
                                                } else {
                                                    $checkedFinalJournalAgendaIds = $journalCandidateIds;
                                                }
                                            @endphp
                                            <div class="border-t border-slate-200 pt-4 dark:border-slate-700">
                                                <p class="splis-label">Apply Final Journal to IV. Committee Report agendas</p>
                                                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                                    Applies this file to tagged agendas’ Journal; agendas can still upload their own Journal/Minutes/Output.
                                                </p>
                                                <div class="mt-3 max-h-56 space-y-2 overflow-y-auto rounded-md border border-slate-200 p-3 dark:border-slate-700">
                                                    @foreach ($finalJournalCandidateAgendas as $candidate)
                                                        <label class="flex items-start gap-2 text-sm text-slate-700 dark:text-slate-300">
                                                            <input
                                                                type="checkbox"
                                                                name="final_journal_agenda_ids[]"
                                                                value="{{ $candidate->id }}"
                                                                class="mt-0.5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-800"
                                                                @checked(in_array((int) $candidate->id, $checkedFinalJournalAgendaIds, true))
                                                            >
                                                            <span>
