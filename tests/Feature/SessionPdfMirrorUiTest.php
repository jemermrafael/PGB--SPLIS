...
class SessionPdfMirrorUiTest extends TestCase
{
    ...
    public function test_edit_page_shows_upload_fields_and_mirror_button(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertOk()
            ->assertDontSee('Upload PDF files', false)
            ->assertDontSee('Google Drive folder link (fallback)', false)
            ->assertDontSee('Final Journal URL (fallback)', false)
            ->assertDontSee('Final Minutes URL (fallback)', false)
            ->assertSee('Google Drive link (.docx)', false)
            ->assertSee('Submitted via', false)
