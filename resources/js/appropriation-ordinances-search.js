...
function renderPdfCell(doc) {
    return `<td class="text-center">
        ${doc.has_pdf
            ? `<a ${pdfModalTriggerAttrs(doc.pdf_url, `${doc.number || 'Appropriation Ordinance'} PDF`)} class="splis-doc-pdf-icon" title="View PDF" aria-label="View PDF">${pdfListIcon}</a>`
...
function renderSubjectCell(doc) {
    const text = String(doc.subject ?? '').trim();
    if (text === '') {
        return '<td><span class="text-slate-400">—</span></td>';
    }

    const { display, full, truncated } = truncateWords(text);

    return `<td>${renderTruncatedTitle(display, full, truncated)}</td>`;
}
...
function renderListItem(doc) {
    return `
        <tr>
            ...
            ${renderSubjectCell(doc)}
            <td class="whitespace-nowrap">${renderDateMeta(formatDate(doc.date_received))}</td>
            <td class="whitespace-nowrap">${renderDateMeta(formatDate(doc.date_passed))}</td>
            <td class="whitespace-nowrap">${renderDateMeta(formatDate(doc.date_approved))}</td>
        </tr>
    `;
}

function renderGridItem(doc) {
    const { display, full, truncated } = truncateWords(String(doc.subject || '').trim());
    ...
    return `
        <article class="splis-doc-card flex flex-col gap-3">
            ...
            <dl class="splis-doc-card-meta">
                <div><dt>Received</dt><dd>${renderDateMeta(formatDate(doc.date_received))}</dd></div>
                <div><dt>Passed by SP</dt><dd>${renderDateMeta(formatDate(doc.date_passed))}</dd></div>
                <div><dt>Approved by Governor</dt><dd>${renderDateMeta(formatDate(doc.date_approved))}</dd></div>
            </dl>
            <div class="mt-auto flex items-center justify-between gap-2 border-t border-slate-100 pt-3 dark:border-slate-700">
                ...
                ${doc.has_pdf
                    ? `<a ${pdfModalTriggerAttrs(doc.pdf_url, `${doc.number || 'Appropriation Ordinance'} PDF`)} class="splis-doc-list-link text-xs font-semibold">View PDF</a>`
...
export function initAppropriationOrdinancesSearch() {
    const root = document.getElementById('appropriation-ordinances-search');
    if (!root) {
        return;
    }

    const form = document.getElementById('appropriation-ordinances-search-form');
    const results = document.getElementById('appropriation-ordinances-search-results');
    const listBody = document.getElementById('appropriation-ordinances-list-body');
    const grid = document.getElementById('appropriation-ordinances-grid');
    const listWrap = document.getElementById('appropriation-ordinances-list-wrap');
    const meta = document.getElementById('appropriation-ordinances-search-meta');
    const pagination = document.getElementById('appropriation-ordinances-search-pagination');
    const viewToggle = document.getElementById('appropriation-ordinances-view-toggle');
    ...
    async function fetchResults() {
        ...
        try {
            ...
            meta.textContent = 'Unable to load appropriation ordinances.';
            listBody.innerHTML = '';
            grid.innerHTML = '';
    ...
    function renderResults(payload) {
        ...
        meta.textContent = `${Number(total || 0).toLocaleString()} Appropriation Ordinance(s) found`;

        if (docs.length === 0) {
            listBody.innerHTML = '<tr><td colspan="6" class="py-12 text-center text-slate-400">No appropriation ordinances match your filters.</td></tr>';
            grid.innerHTML = '<p class="col-span-full py-12 text-center text-slate-400">No appropriation ordinances match your filters.</p>';
            pagination.innerHTML = '';
            return;
        }
