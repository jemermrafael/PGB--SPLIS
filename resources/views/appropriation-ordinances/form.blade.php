...
<div class="max-w-3xl">
    ...
    <form method="POST" action="{{ $isEdit ? route('appropriation-ordinances.update', $appropriationOrdinance) : route('appropriation-ordinances.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @if ($isEdit)
            @method('PUT')
        @endif
